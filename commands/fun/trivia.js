exports.run = async (client, message, args) => {
	if (!sentTrivia.has(message.guild.id)) {
		embed = new Discord.MessageEmbed().setTitle(`Generating`).setColor(0xffa21c);
		let question = await message.channel.send(embed);
		fetch('https://opentdb.com/api.php?amount=1&encode=base64').then((res) => res.json()).then((body) => {
			sentTrivia.add(message.guild.id);
			embed = new Discord.MessageEmbed()
				.setTitle(
					`${Buffer.from(body.results[0].category, 'base64').toString()} - ${Buffer.from(
						body.results[0].difficulty,
						'base64'
					)
						.toString()
						.toUpperCase()}`
					...
					client.channels.cache
						...
						.awaitMessages(
							(response) =>
								...
								Buffer.from(body.results[0].correct_answer, 'base64').toString().toLowerCase(),
							{
								max: 1,
								time: 30000,
								errors: [ 'time' ]
						...
						.then((collected) => {
							...
							embed = new Discord.MessageEmbed()
								...
								.setColor(0x01c818);
							question.edit(embed).catch((err) => {
								return;
							});
							if (Buffer.from(body.results[0].difficulty, 'base64').toString().toUpperCase() == 'EASY') {
								points = 10;
							} else if (
								Buffer.from(body.results[0].difficulty, 'base64').toString().toUpperCase() == 'MEDIUM'
							) {
								points = 25;
							} else {
								points = 50;
							...
							client.channels.cache
								...
								.send(
									`Correct ${collected
										.first()
										.author.toString()}! You scored ${points} <:bells:698107158805348373>.`
								);
							let score;
						...
						.catch(() => {
							...
							return client.channels.cache
								.get(message.channel.id)
								.send(
									`No one answered correctly, the answer was ${Buffer.from(
	...
	} else client.channels.cache.get(message.channel.id).send(`A question has already been asked.`);
};
