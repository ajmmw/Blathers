...
module.exports = (client) => {
	...
	client.setScore = UserSQL.prepare(
		'INSERT OR REPLACE INTO scores (id, user, guild, name, points, level) VALUES (@id, @user, @guild, @name, @points, @level);'
	);

	//User Islands
	client.getIsland = UserSQL.prepare('SELECT * FROM islands WHERE id = ?');
	client.setIsland = UserSQL.prepare(
		'INSERT OR REPLACE INTO islands (id, character, name, fruit, hemisphere) VALUES (@id, @character, @name, @fruit, @hemisphere);'
	);
